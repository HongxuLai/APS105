...
bool isValidMove(const char board[][26], int n, int row, int col, char colour);
void applyMove(char board[][26], int n, int row, int col, char colour);
int countFlipsInDirection(const char board[][26], int n, int row, int col, char colour, int deltaRow, int deltaCol);
int getMoveScore(const char board[][26], int n, int row, int col, char colour);
void getBestMove(const char board[][26], int n, char colour, int *bestRow, int *bestCol);
char winner(const char board[][26], int n);
void computerMove(char board[][26], int n, char complayer);
bool playerMove(char board[][26], int n, char player);
bool hasValidMove(const char board[][26], int n, char colour);
...
void computerMove(char board[][26], int n, char complayer)
{
    ...
    if(bestRow != -1)
    {
        ...
        applyMove(board, n, bestRow, bestCol, complayer);
    }
}
...
bool playerMove(char board[][26], int n, char player)
{
    ...
    {
        applyMove(board, n, row, col, player);
        return true;
    }
    else
    {
...
void copyBoard(const char src[][26], char dst[][26], int n)
{
    for (int r = 0; r < n; r++)
    {
        for (int c = 0; c < n; c++)
...
int countValidMoves(const char board[][26], int n, char colour)
{
    int count = 0;
    for (int r = 0; r < n; r++)
    {
...
int evaluateMove(const char board[][26], int n, int row, int col, char turn)
{
    ...
    char temp[26][26];
    copyBoard(board, temp, n);
    applyMove(temp, n, row, col, turn);

    char opponent = (turn == 'B') ? 'W' : 'B';

    int flips = getMoveScore(board, n, row, col, turn);
    int myMoves = countValidMoves(temp, n, turn);
    int opponentMoves = countValidMoves(temp, n, opponent);
    ...
    if (n == 8)
    {
        ...
        positional = weight[row][col];
    }

    return positional + flips * 2 + myMoves * 2 - opponentMoves * 5;
}
...
int makeMove(const char board[][26], int n, char turn, int *row, int *col)
{
    ...
    *col = -1;
    ...
    for (int r = 0; r < n; r++)
    {
        for (int c = 0; c < n; c++)
        {
            if (isValidMove(board, n, r, c, turn))
            {
                int score = evaluateMove(board, n, r, c, turn);
                if (score > bestScore)
                {
                    bestScore = score;
                    *row = r;
